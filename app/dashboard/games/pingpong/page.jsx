...
const PingPongGame = () => {
  ...
  const storeGameMove = async (move, score) => {
    try {
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      setGameState(prev => {
        ...
        // Store game state on chain if score changed
        if (newScore1 !== prev.score1 || newScore2 !== prev.score2) {
          storeGameMove({
            ballPos: { x: finalBallX, y: finalBallY },
            paddle1: gameState.paddle1Y,
            paddle2: newPaddle2Y,
            scoreUpdate: { player: newScore1, ai: newScore2 }
          }, Math.max(newScore1, newScore2));
        }
