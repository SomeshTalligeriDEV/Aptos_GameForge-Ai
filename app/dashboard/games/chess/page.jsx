...
const ChessBlitz = () => {
  ...
  const storeGameMove = async (move, currentMoves) => {
    try {
  ...
  const makeAIMove = useCallback(() => {
    if (game.turn() !== playerColor[0] && gameStatus === 'playing') {
      ...
      aiWorkerRef.current.onmessage = async (e) => {
        try {
          ...
          // Store AI move on chain
          await storeGameMove(move, newMoveCount);
