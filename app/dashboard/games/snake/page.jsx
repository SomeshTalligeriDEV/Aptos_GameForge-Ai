...
const SnakeGame = () => {
  ...
  const [gameId] = useState(() => Math.floor(Math.random() * 1000000));
  
  // Store game move on chain
  const storeGameMove = async (snakeState, currentScore) => {
    try {
      await fetch('/api/games/store', {
        ...
        body: JSON.stringify({
          ...
          moves: snakeState,
          score: currentScore,
          walletAddress: window.aptos?.account
        }),
      });
    } catch (error) {
      console.error('Error storing game move:', error);
    }
  };
  ...
  useEffect(() => {
    ...
    const moveSnake = () => {
      ...
      if (ateFruit) {
        ...
        storeGameState(newSnake, newScore);
      }
    };

    // Store game state on chain
    const storeGameState = async (snakeState, currentScore) => {
      try {
        const response = await fetch('/api/snake/store', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            gameId: 1, // You'll need to get the actual game ID
            moves: snakeState,
            score: currentScore,
            walletAddress: 'YOUR_WALLET_ADDRESS' // You'll need to get the actual wallet address
          }),
        });
